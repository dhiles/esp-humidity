...
// =================================================================
// USER CONFIGURATION (Ensure these match your wiring/address)
// =================================================================
...
void i2c_master_init(void)
{
    i2c_config_t conf = {
        ...
        .sda_pullup_en = GPIO_PULLUP_ENABLE, // Recommended change
        .scl_io_num = I2C_MASTER_SCL_IO,
        .scl_pullup_en = GPIO_PULLUP_ENABLE, // Recommended change
...
void i2c_scanner1(void)
{
    ...
    for (u8 addr = 0x08; addr <= 0x77; addr++)
    {
        ...
        if (res == ESP_OK)
        {
            if (test_data == 0x60)
            {
                ...
                found = true;
                // If the default address is correct, break early
                if (addr == BME280_I2C_ADDR)
                    break;
            }
            else if (test_data != 0x00 && test_data != 0xFF)
            {
    ...
    if (!found)
    {
        ESP_LOGE(TAG, "No I2C devices found—check wiring, power, pull-ups, and address!");
    }
}

void i2c_scanner2(void)
{
    ESP_LOGI(TAG, "Starting continuous I2C bus scan for voltage testing...");

    // Add an outer 'while(1)' loop to run indefinitely
    while (1)
    {
        bool found = false;
        ...
        for (u8 addr = 0x08; addr <= 0x77; addr++)
        {
            ...
            // Attempt to read the CHIP ID (0xD0)
            i2c_master_start(cmd);
            i2c_master_write_byte(cmd, (addr << 1) | I2C_MASTER_WRITE, true);
            ...
            if (res == ESP_OK)
            {
                if (test_data == 0x60)
                {
                    ESP_LOGI(TAG, "BME280 confirmed at address 0x%02X (CHIPID=0x60)", addr);
                    // ... (found = true; break) ...
                }
                // Check for BMP280 ID (0x58)
                else if (test_data == 0x58)
                {
                    ESP_LOGW(TAG, "BMP280 confirmed at address 0x%02X (CHIPID=0x58). Note: This is not a BME280.", addr);
                    // ... (found = true; break) ...
                }
                // Log ANY device that acknowledges the address and has a valid ID
                else if (test_data != 0x00 && test_data != 0xFF)
                {
                    ESP_LOGI(TAG, "Device found at address 0x%02X (ID=0x%02X)", addr, test_data);
                }
        ...
        // After checking all addresses, wait before restarting the full scan.
        // This ensures the bus spends most of its time in the HIGH (idle) state.
