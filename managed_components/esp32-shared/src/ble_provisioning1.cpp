...
#define MAX_MSG_LEN 128   // Max message size for queue
    ...
    // WiFi Control Characteristic UUIDs
    ...
    static const struct ble_gatt_chr_def provisioning_chars[] = {
        ...
        {
            // WiFi Enable (write 1 = on, 0 = off)
            .uuid = &WIFI_ENABLE_CHAR_UUID.u,
            .access_cb = ble_provisioning_access_cb,
            .flags = BLE_GATT_CHR_F_WRITE,
            .val_handle = &wifi_enable_handle,
        },
        {
            // WiFi Status (read only)
            .uuid = &WIFI_STATUS_CHAR_UUID.u,
            .access_cb = ble_provisioning_access_cb,
            .flags = BLE_GATT_CHR_F_READ,
            .val_handle = &wifi_status_handle,
        },
        {
            // WiFi Toggle (write anything to toggle)
            .uuid = &WIFI_TOGGLE_CHAR_UUID.u,
            .access_cb = ble_provisioning_access_cb,
            .flags = BLE_GATT_CHR_F_WRITE,
            .val_handle = &wifi_toggle_handle,
        },
        {
            .uuid = &DEVICE_ID_CHAR_UUID.u,
            .access_cb = ble_provisioning_access_cb,
            .flags = BLE_GATT_CHR_F_READ,
            .val_handle = &device_id_handle,
        },
        {0}, // Stop marker
    };
                                          ...
                                          struct ble_gatt_access_ctxt *ctxt, void *arg)
    {
        ...
        if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR)
        {
            ...
            else if (attr_handle == wifi_status_handle)
            {
                ...
                ESP_LOGI(TAG, "Read WiFi status: %s", status ? "ON" : "OFF");
                rc = os_mbuf_append(ctxt->om, &status, 1);
                return (rc == 0) ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
            }
            else if (attr_handle == device_id_handle)
            {
                const char *id = MySystem::get_device_id(); // e.g. "M9K4P2X7"
                ESP_LOGI(TAG, "Client read Device ID: %s", id);
                rc = os_mbuf_append(ctxt->om, id, strlen(id));
                return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
            }
            else
            {
                ESP_LOGW(TAG, "Read not permitted on handle %d", attr_handle);
        ...
        if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR)
        {
            ...
            else if (attr_handle == wifi_enable_handle)
            {
                if (len >= 1)
                {
                    ...
                    ESP_LOGI(TAG, "WiFi %s via BLE", enable ? "ENABLED" : "DISABLED");
            ...
            else if (attr_handle == wifi_toggle_handle)
            {
                ...
                ESP_LOGI(TAG, "WiFi TOGGLED via BLE → now %s", now_on ? "ON" : "OFF");
                send_notification_safe(now_on ? "WiFi: ON" : "WiFi: OFF");
            }
            else
            {
                ESP_LOGW(TAG, "Write to unknown handle %d", attr_handle);
